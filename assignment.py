...
class Assignment:
    ...
    def _getName(self, assignment: Locator) -> str:
        ...
        """
        try:
            self.name: str = assignment.locator("a.d2l-link[title^='Submit'] strong, .d2l-foldername:nth-child(1) strong").first.inner_text()
        except:
            self.name: str = "Unavailable"
    ...
    def _getGrade(self, assignment: Locator) -> str | None:
        ...
        # Set the grade
        self.grade = ""
        for i in range(len(partsOfGrade)):
            self.grade += partsOfGrade[i].get_text()
    ...
    def _getFeedback(self, assignment: Locator, page: Page) -> dict[str, str] | None:
        ...
        feedbackHTML = feedbackTables[0].select("tr:last-child .d2l-htmlblock-untrusted > d2l-html-block")
